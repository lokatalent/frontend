...
const DashboardPaymentPage = () => {
  ...
  const onAddProvider = async (id: any) => {
    ...
    if (!response.error) {
      setLoading(false);
      dispatch(showToast({status:"success", message: "Request sent successfully."}));
  ...
  const paystackFunction = async (code: any, id: any, ref: any) => {
     ...
     const callbacks = {
        onSuccess: () => {
          if (!intervalID) {
            intervalID = setInterval(onVerifyPayment, 1000, id, ref);
            // setIntervalID(fID);
          }
        },
        onCancel: () =>
  ...
  return (
    bookingData?.payment_status === "verified" ? onAddProvider(bookingData?.id) :
          ...
          <div className="bg-white w-full py-4 sm:py-6 px-4 sm:px-8 rounded-md order-1 lg:order-2">
            ...
            <div className="flex flex-row items-center gap-4 sm:gap-6 mb-4 sm:mb-5">
              <div className="relative h-24 w-24 sm:h-40 sm:w-40 shrink-0">
                <Image
                  src={talentData.provider_details.avatar || "/Images/camera.png"}
                  fill
