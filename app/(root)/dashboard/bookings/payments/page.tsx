...
const DashboardPaymentPage = () => {
  ...
  // Big change!
  // The app doesn't build with the inline-js library. Perhaps you can fix that.
  // Sync with the BE to discuss on the new flow for accepting payments which utilizes the redirect method for Paystack.
  ...
  const paystackFunction = (code: any, id: any) => {
  ...
  const onMakePayment = async (id: any) => {
    ...
    if (!response.error) {
      ...
      paystackFunction(response.data, id);
    } else {
      setLoading(false);
      if (response.status === 401) {
        dispatch(
